...
@click.option("--logs/--no-logs", default=True, help="Export tensorflow log files.")
@click.option(
    "--checkpoint",
    default=None,
    help="Name (usually of form 'model-N') of the checkpoint to use. Defaults to latest checkpoint",
)
def main(config_files, output_folder, data_settings, logs, checkpoint):
    ...
    # First try to load a different checkpoint via 'checkpoint' command line option
    ...
    # -----------------------------
    export_data_settings(data_settings=data_settings, output_folder=output_folder)
    ...
    if logs:
        ...
        shutil.copytree(src=log_directory, dst=os.path.join(output_folder, "logs"))
...
def export_data_settings(data_settings, output_folder):
    ...
    if "DNN_partial_exclusion" in data_config:
        data_settings["partial_exclusion"] = data_config["DNN_partial_exclusion"]
    ...
    with open(os.path.join(output_folder, "config_data_settings.yaml"), "w") as f:
